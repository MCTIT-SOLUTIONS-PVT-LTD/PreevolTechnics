...
Public Class XR_ExportServiceIGSTWithoutDis
    ...
    Dim daTotal As OleDbDataAdapter
    ...
    Private Sub XrTable1_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles XrTable1.BeforePrint
        sqlIgst = "SELECT DISTINCT IGSTRate from ServiceInvoiceDetail Where ServiceInvoiceID = " + Invoice.Value.ToString()
        sqlLineItems = "SELECT TaxableValue,IGSTRate,IGSTAmt from ServiceInvoiceDetail Where ServiceInvoiceID = " + Invoice.Value.ToString()
        sqlPacking = "SELECT PackingCharge,PIGSTRate,PIGSTAmt from ServiceInvoice Where ServiceInvoiceID = " + Invoice.Value.ToString()
    ...
    Private Sub XrLabel1_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles XrLabel1.BeforePrint
        XrLabel1.Text = "(""Supply meant For Export Without payment Of IGST"")"
    End Sub

    Private Sub TaxableValueLabel_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles TaxableValueLabel.BeforePrint
        TaxableValueLabel.Text = "$" + Math.Round(SumTaxVal, 2).ToString("F2")
    End Sub

    Private Sub IGSTAmtLabel_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles IGSTAmtLabel.BeforePrint
        IGSTAmtLabel.Text = "$" + Math.Round(SumIGST, 2).ToString("F2")
    End Sub

    Private Sub TotalLabel_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles TotalLabel.BeforePrint
        TotalLabel.Text = "$" + Math.Round(SumTotal, 2).ToString("F2")
    End Sub
    ...
    Private Sub XrTable2_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles XrTable2.BeforePrint
        'query to fetch total amount display in label
        sqlTotal = "SELECT PackingCharge,PIGSTAmt,TotalGSTTax,TotalAmtAfterTax,RoundOff,GrandTotalAmount from ServiceInvoice Where ServiceInvoiceID = " + Invoice.Value.ToString()
        daTotal = New OleDbDataAdapter(sqlTotal, ConStr)
        daTotal.Fill(dtTotal)
        Dim packing As Decimal
        packing = Convert.ToDecimal(dtTotal.Rows(0).Item(0))
        PackingChargeCell.Text = "$" + Math.Round(packing, 2).ToString("F2")
        Dim packingGst As Decimal
        packingGst = Convert.ToDecimal(dtTotal.Rows(0).Item(1))
        PackingGSTCell.Text = "$" + Math.Round(packingGst, 2).ToString("F2")
        Dim totaltax As Decimal
        totaltax = Convert.ToDecimal(dtTotal.Rows(0).Item(2))
        TotalGSTCell.Text = "$" + Math.Round(totaltax, 2).ToString("F2")
        Dim amtafter As Decimal
        amtafter = Convert.ToDecimal(dtTotal.Rows(0).Item(3))
        TaxableValueCell.Text = "$" + Math.Round(amtafter, 2).ToString("F2")
        Dim roundoff As Decimal
        roundoff = Convert.ToDecimal(dtTotal.Rows(0).Item(4))
        RoundOffCell.Text = "$" + Math.Round(roundoff, 2).ToString("F2")
        Dim grandtotal As Decimal
        grandtotal = Convert.ToDecimal(dtTotal.Rows(0).Item(5))
        GrandTotalCell.Text = "$" + Math.Round(grandtotal, 2).ToString("F2")

        'Packing charges and gst values from PackingCharges datatable
    ...
    Private Sub XrPictureBox3_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles XrPictureBox3.BeforePrint
        ...
        sqlQRCode = "SELECT QRCode,IRNNo,AckNo,AckDate from ServiceInvoice Where ServiceInvoiceID = " + Invoice.Value.ToString()
    ...
    Private Sub XrTable4_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles XrTable4.BeforePrint
        ...
        Dim dtIgst As New DataTable
        Dim sqlIgst As String
        sqlIgst = "SELECT ProductName, Description, HSNACS, UOM, Qty, Rate, Amount, Discount, DiscountVal, TaxableValue, IGSTRate, IGSTAmt, TotalAmount from ServiceInvoiceDetail Where ServiceInvoiceID = " + Invoice.Value.ToString()
        Try
            daIgst = New OleDbDataAdapter(sqlIgst, ConStr)
            daIgst.Fill(dtIgst)
