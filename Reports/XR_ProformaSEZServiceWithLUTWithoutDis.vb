...
Public Class XR_ProformaSEZServiceWithLUTWithoutDis
    ...
    Private Sub XrTable1_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles XrTable1.BeforePrint
        ...
        'Dim i, j As Integer
        sqlIgst = "SELECT DISTINCT IGSTRate from ProformaServiceInvoiceDetail Where ProformaServiceInvoiceID = " + Invoice.Value.ToString()
        sqlLineItems = "SELECT TaxableValue,IGSTRate,IGSTAmt from ProformaServiceInvoiceDetail Where ProformaServiceInvoiceID = " + Invoice.Value.ToString()
        sqlPacking = "SELECT PackingCharge,PIGSTRate,PIGSTAmt from ProformaServiceInvoice Where ProformaServiceInvoiceID = " + Invoice.Value.ToString()
    ...
    Private Sub XrLabel1_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles XrLabel1.BeforePrint
        XrLabel1.Text = "(""Supply under LUT number________ without payment of IGST"")"
    End Sub
    ...
    Private Sub XrTable4_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles XrTable4.BeforePrint
        ...
        Dim dtIgst As New DataTable
        Dim sqlIgst As String
        sqlIgst = "SELECT ProductName, Description, HSNACS, UOM, Qty, Rate, Amount, Discount, TaxableValue, IGSTRate, IGSTAmt, TotalAmount from ProformaServiceInvoiceDetail Where ProformaServiceInvoiceID = " + Invoice.Value.ToString()
        Try
            daIgst = New OleDbDataAdapter(sqlIgst, ConStr)
            daIgst.Fill(dtIgst)
            ...
            For i = 0 To dtIgst.Rows.Count - 1
                ...
                taxableVal = Convert.ToDecimal(dtIgst.Rows(i).Item(8))
                ...
                Dim ttl As Decimal
                ttl = Convert.ToDecimal(dtIgst.Rows(i).Item(8))
    ...
    'Private Sub TotalAmtLI_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs)
    '    TotalAmtLI.Text = TaxableValLI.Text
    'End Sub
    Private Sub XrTable3_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles XrTable3.BeforePrint
        Dim daTotalAmtBfrTax As OleDbDataAdapter
        Dim dtTotalAmtBfrTax As New DataTable
        Dim sqlTotalAmtBfrTax As String
        sqlTotalAmtBfrTax = "SELECT TotalAmtBeforeTax from ProformaServiceInvoice Where ProformaServiceInvoiceID = " + Invoice.Value.ToString()
        daTotalAmtBfrTax = New OleDbDataAdapter(sqlTotalAmtBfrTax, ConStr)
        daTotalAmtBfrTax.Fill(dtTotalAmtBfrTax)
        Dim TotalTaxVal As Decimal
        For i = 0 To dtTotalAmtBfrTax.Rows.Count - 1
            TotalTaxVal = Convert.ToDecimal(dtTotalAmtBfrTax.Rows(0).Item(0))
        Next
        TaxableValueCell.Text = TotalTaxVal.ToString("F2")
        GrandTotalCell.Text = Math.Round(CDec(TaxableValueCell.Text)).ToString("F2")
        RoundOffCell.Text = (CDec(GrandTotalCell.Text) - CDec(TaxableValueCell.Text)).ToString("F2")
    End Sub
End Class