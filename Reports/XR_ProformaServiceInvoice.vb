...
Public Class XR_ProformaServiceInvoice
    Private Sub XrTable3_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles XrTable3.BeforePrint
        ...
        Dim i, j As Integer
        sqlCgst = "SELECT DISTINCT CGSTRate from ProformaServiceInvoiceDetail Where ProformaServiceInvoiceID = " + Invoice.Value.ToString()
        sqlLineItems = "SELECT TaxableValue,CGSTRate,CGSTAmt,SGSTRate,SGSTAmt from ProformaServiceInvoiceDetail Where ProformaServiceInvoiceID = " + Invoice.Value.ToString()
        sqlPacking = "SELECT PackingCharge,PCGSTRate,PCGSTAmt,PSGSTRate,PSGSTAmt from ProformaServiceInvoice Where ProformaServiceInvoiceID = " + Invoice.Value.ToString()
    ...
    Private Sub XrTable2_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles XrTable2.BeforePrint
        ...
        Dim dtCgst As New DataTable
        Dim sqlCgst As String
        sqlCgst = "SELECT ProductName, Description, HSNACS, UOM, Qty, Rate, Amount, Discount, DiscountVal, TaxableValue, CGSTRate, CGSTAmt, SGSTRate, SGSTAmt, TotalAmount from ProformaServiceInvoiceDetail Where ProformaServiceInvoiceID = " + Invoice.Value.ToString()
        Try
            daCgst = New OleDbDataAdapter(sqlCgst, ConStr)
            daCgst.Fill(dtCgst)
