...
Public Class XR_ExportSalesIGSTWithoutDis
    ...
    Private Sub XrTable4_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles XrTable4.BeforePrint
        ...
        Try
            ...
            For i = 0 To dtIgst.Rows.Count - 1
                ...
                igstRate.Text = dtIgst.Rows(i).Item(9).ToString() + "%"
                igstRate.TextAlignment = TextAlignment.MiddleRight
                igstRate.WidthF = 26

                igstAmt.Borders = BorderSide.Bottom Or BorderSide.Top Or BorderSide.Left
                Dim iamt As Decimal
                iamt = Convert.ToDecimal(dtIgst.Rows(i).Item(10))
                ...
                Dim ttl As Decimal
                ttl = Convert.ToDecimal(dtIgst.Rows(i).Item(11))
    ...
    Private Sub TaxableValueLabel_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles IGSTAmtLabel.BeforePrint
        TaxableValueLabel.Text = "$" + Math.Round(SumTaxVal, 2).ToString("F2")
    End Sub

    Private Sub IGSTAmtLabel_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs)
    ...
    Private Sub XrTable3_BeforePrint(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles XrTable3.BeforePrint
        ...
        sqlTotal = "SELECT TotalGSTTax,TotalAmtAfterTax,RoundOff,GrandTotalAmount from Invoice Where InvoiceID = " + Invoice.Value.ToString()
        daTotal = New OleDbDataAdapter(sqlTotal, ConStr)
        daTotal.Fill(dtTotal)
        Dim totaltax As Decimal
        totaltax = Convert.ToDecimal(dtTotal.Rows(0).Item(0))
        TotalGSTCell.Text = "$" + Math.Round(totaltax, 2).ToString("F2")
        Dim amtafter As Decimal
        amtafter = Convert.ToDecimal(dtTotal.Rows(0).Item(1))
        TaxableValueCell.Text = "$" + Math.Round(amtafter, 2).ToString("F2")
        Dim roundoff As Decimal
        roundoff = Convert.ToDecimal(dtTotal.Rows(0).Item(2))
        RoundOffCell.Text = "$" + Math.Round(roundoff, 2).ToString("F2")
        Dim grandtotal As Decimal
        grandtotal = Convert.ToDecimal(dtTotal.Rows(0).Item(3))
        ...
        packing = Convert.ToDecimal(dtPacking.Rows(0).Item(0))
        PackingChargeCell.Text = "$" + Math.Round(packing, 2).ToString("F2")
        Dim packingGst As Decimal
        packingGst = Convert.ToDecimal(dtPacking.Rows(0).Item(2))
