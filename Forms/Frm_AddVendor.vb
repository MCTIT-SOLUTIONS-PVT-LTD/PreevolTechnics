...
Public Class Frm_AddVendor
    ...
    Function SetDataTable()
        ...
        With DS.Tables("Vendor").Columns("VendorID")
            .AutoIncrement = True
            .AutoIncrementSeed = -1
            .AutoIncrementStep = -1
            .ReadOnly = True
    ...
    Function SetQuery()
        DA.SelectCommand = New OleDbCommand("Select VendorID,VendorName,ContactPerson,Contact1,Contact2,Address,City,State,StateCode,Email,Website,GSTStatus,GSTIN,PANNo,VATNo,CSTNo,ECCNo,IECCode,Company From Vendor Where VendorID=@VendorID", ConStr)
        DA.SelectCommand.Parameters.Add("@VendorID", OleDbType.Integer, 4, "VendorID")

        DA.InsertCommand = New OleDbCommand("Insert Into Vendor (VendorName,ContactPerson,Contact1,Contact2,Address,City,State,StateCode,Email,Website,GSTStatus,GSTIN,PANNo,VATNo,CSTNo,ECCNo,IECCode,Company) Values (@VendorName,@ContactPerson,@Contact1,@Contact2,@Address,@City,@State,@StateCode,@Email,@Website,@GSTStatus,@GSTIN,@PANNo,@VATNo,@CSTNo,@ECCNo,@IECCode,@Company)", ConStr)
        DA.InsertCommand.Parameters.Add("@VendorName", OleDbType.VarChar, 80, "VendorName")
        ...
        DA.UpdateCommand = New OleDbCommand("Update Vendor Set VendorName=@VendorName,ContactPerson=@ContactPerson,Contact1=@Contact1,Contact2=@Contact2,Address=@Address,City=@City,State=@State,StateCode=@StateCode,Email=@Email,Website=@Website,GSTStatus=@GSTStatus,GSTIN=@GSTIN,PANNo=@PANNo,VATNo=@VATNo,CSTNO=@CSTNO,ECCNo=@ECCNo,IECCode=@IECCode Where VendorID=@VendorID", ConStr)
        DA.UpdateCommand.Parameters.Add("@VendorName", OleDbType.VarChar, 80, "VendorName")
        ...
        DA.UpdateCommand.Parameters.Add("@VendorID", OleDbType.Integer, 4, "VendorID")

        DA.DeleteCommand = New OleDbCommand("Delete From Vendor Where VendorID=@VendorID", ConStr)
        DA.DeleteCommand.Parameters.Add("@VendorID", OleDbType.Integer, 4, "VendorID")
        Return True
    End Function

    Function SetBinding()
        BS.DataSource = DS.Tables("Vendor")
        VendorNameTextEdit.DataBindings.Add(New Binding("EditValue", BS, "VendorName"))
    ...
    Private Sub OpenBarButtonItem_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles OpenBarButtonItem.ItemClick
        Dim ID = ShowRecord("Select * From Vendor Where Company = '" + PubCompanyName + "'", "VendorID")
        If ID > 0 Then
            Try
                DS.Tables("Vendor").Clear()
            Catch
            End Try
            DA.SelectCommand.Parameters("@VendorID").Value = ID
            DA.Fill(DS, "Vendor")
        End If
    End Sub
    ...
    Function Validation() As Boolean
        If VendorNameTextEdit.EditValue Is DBNull.Value Or VendorNameTextEdit.EditValue Is "" Then
            VendorNameTextEdit.Focus()
            Return False
    ...
    Private Sub SaveSimpleButton_Click(sender As Object, e As EventArgs) Handles SaveSimpleButton.Click
        If Validation() Then
            ...
            XtraMessageBox.Show("Vendor added successfully...", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information)
            AddNew()
        End If
    End Sub
    ...
    Private Sub AddressMemoEdit_Validating(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles AddressMemoEdit.Validating
        If Not AddressMemoEdit.EditValue Is DBNull.Value Then
            Try
                Dim CMD As New OleDbCommand("SELECT Count(VendorID) FROM Vendor WHERE VendorName=@VendorName AND Address=@Address AND VendorID<>@VendorID", ConStr)
                CMD.Parameters.AddWithValue("@VendorName", VendorNameTextEdit.EditValue)
                CMD.Parameters.AddWithValue("@Address", AddressMemoEdit.EditValue)
                CMD.Parameters.AddWithValue("@VendorID", IIf(BS.Current!VendorID Is DBNull.Value, -1, BS.Current!VendorID))
